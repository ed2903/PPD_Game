// LEVEL EDITOR NOTES: 
// Couldn't be arsed removing half the code so this file is like 3x longer than it should be
// half the code is not used although the base game is used and edited so dont go deleting shit 
// --------------------------------------
...
var defaultLineWidth = 0.5;
// holds the coordinates of the mouse in a vector
var mouseCoords = new Victor(0,0);

var angle = 0;
var segmentGrowthSpeed = 1;
var csHitsToBreak = 1;
var lockMouse = true;
var segmentJustAdded = false;
var segmentJustDeleted = false;
...
var bar  = new Bar(new Victor(game.width / 2,game.height / 2),(game.width / 2) - 40,toRadians(30),toRadians(-15),getColor(csHitsToBreak));
...
document.onkeydown = function(e) {
    ...
    switch (e.keyCode) {
        ...
        case 65:
            {		
            	if (bar.angle - toRadians(segmentGrowthSpeed) > 0)
            	{
	            	bar.angle -= toRadians(segmentGrowthSpeed);
	            	segmentGrowthSpeed+= 0.01;
            	}
            }
            break;
        case 87:
            {
            	bar.radius += segmentWidth;
            }
            break;
        case 68:
            {
        		if (bar.angle + toRadians(segmentGrowthSpeed) < Math.PI * 2)
            	{
	            	bar.angle += toRadians(segmentGrowthSpeed);
	            	segmentGrowthSpeed+= 0.01;
            	}
        ...
        case 81:
        	{
				...
				console.info(csHitsToBreak);
				bar.color = getColor(csHitsToBreak);
    			
        	}
        	break;
        case 69:
    		{	
    			if (segmentJustAdded == false)
    			{
    				segments.push(new Segment(new Victor(game.width / 2, game.height / 2),bar.radius,bar.startAngle,bar.endAngle,csHitsToBreak));
    				segmentJustAdded = true;
    			}
    		}
    		break;
    	case 82:
			{
    			if (segmentJustDeleted == false)
    			{
    				segments.pop();
    				segmentJustDeleted = true;
    			}
			}
			break;
    	default:
...
document.onkeyup = function(e) {
    ...
    switch (e.keyCode) {
        ...
        case 69:
        	{
        		if (segmentJustAdded == true)
        		{
        			segmentJustAdded = false;
        		}
        ...
        case 82:
        	{
        		if (segmentJustDeleted == true)
        		{
        			segmentJustDeleted = false;
        		}
        	}
        	break;
        case 13:
...
function DrawGuideLines(){
	...
	for (i=segmentWidth / 2; i < game.width / 2; i += segmentWidth)
	{
		ctx.lineWidth = defaultLineWidth
		ctx.strokeStyle = '#000000';
		ctx.beginPath();
		ctx.arc(game.width / 2,game.height / 2,i,0,Math.PI * 2,'#000000');
		ctx.stroke();
	}
